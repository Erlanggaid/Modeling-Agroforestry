...
first_decision_function <- function(x, varnames) {
  ...
  NPV_tradeoff_FL <- discount(tradeoff_FL, discount_rate = discount_rate, calculate_NPV = TRUE)
  
  # In the return list, one can indicate any outcome they wish to see from the model
  return(list(trade_off_FL = NPV_tradeoff_FL,
...
#### Model Function 2 (Rubber Monoculture vs Rubber-Archidendron jiringa (Jengkol) and Parkia speciosa (Petai))
```{r}
...
second_decision_function <- function(x, varnames) {
 ...
 # Equipment cost including 3 and 5 years replacement
  ...
  # Calculate The Total Benefit of Rubber Monoculture
  ...
  # NPV comparison (excluding family labour calculation)
  ...
  tradeoff_benefit_FL <- NPV_rb_ag2.efl - NPV_rb.efl
  
  # Final NPV of the decision to choose
  
  NPV_tradeoff_FL <- discount(tradeoff_benefit_FL, discount_rate = discount_rate, calculate_NPV = TRUE)
...
third_decision_function <- function(x, varnames) {
  ...
  # Benefits of Land Clearing for Wood Harvesting
  benefit.land.clearing.rubber[1] <- vv(benefit_land_clearing, var_CV, 1)
  benefit.land.clearing.rubber[2:n_years] <- 0
  ...
  # Calculate The Total Benefit of Rubber Monoculture (excluding family labour calculation)
    
  total_rb_benefit.efl <- (tot_rb_benefit + benefit.land.clearing.rubber - seedling.cost.rubber - land.clearing.rubber - 
                           fertilizer.cost.rubber - equipment.cost - harvest.cost.rubber - transportation.cost.rubber - 
                           monitoring.cost.rubber - herbicide.cost.rubber + tot_banana_benefit.efl + 
                           tot_cassava_benefit.efl + tot_chilli_benefit.efl)/cur_change
  ...
  tradeoff_benefit_FL <- NPV_rb_ag3.efl - NPV_rb.efl
...
```{r echo=TRUE, warning=FALSE, fig.cap="**Graph. 1 Cashflow Rubber Areca nut Durian (Excluding Family Labour Calculation)**",}
...
```{r echo=TRUE, fig.cap="**Graph. 2. Cash flow Rubber-Archidendron j.-Parkia s. (Excluding Family Labour Calculation)**",}
...
```{r echo=TRUE, warning=FALSE, fig.cap="**Graph. 3. Cash flow Rubber-Archidendron j.-Sandoricum k. (Excluding Family Labour Calculation)**",}
...
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result1, 
                                    vars = c("rb.efl_NPV", "rb_ag.efl_NPV"),
                                    method = 'smooth_simple_overlay', 
                                    bins = 150,
                                    old_names = NULL,
                                    new_names = NULL,
                                    colors = c("darkolivegreen2", "darkorange3"),
                                    outlier_shape = ".",
...
### NPV Rubber monoculture vs Rubber, Archidendron, and jiringa-Parkia speciosa
...
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result2, 
                                    vars = c("rb.efl_NPV", "rb_ag2.efl_NPV"),
                                    method = 'smooth_simple_overlay', 
                                    bins = 150,
                                    old_names = NULL,
                                    new_names = NULL,
                                    colors = c("darkolivegreen2", "springgreen4"),
                                    outlier_shape = ".",
                                    x_axis_name = "Outcome distribution",
                                    y_axis_name = "frequency",
                                    base_size = 11)
...
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result3, 
                                    vars = c("rb_ag3.efl_NPV", "rb.efl_NPV"),
                                    method = 'smooth_simple_overlay', 
                                    bins = 150,
                                    old_names = NULL,
                                    new_names = NULL,
                                    colors = c("darkolivegreen2", "mediumorchid2"),
                                    outlier_shape = ".",
                                    x_axis_name = "Outcome distribution",
                                    y_axis_name = "frequency",
                                    base_size = 11)
...
```{r echo=TRUE, warning=FALSE, fig.cap="**Graph. 8. Tradeoff Rubber-Areca nut-Durian over Rubber Monoculture**",}
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result1, 
                                    vars = "trade_off_FL",
                                    method = 'boxplot',
                                    colors = "goldenrod1",
...
### Trade off Rubber, Archidendron jiringa and Parkia speciosa over Rubber monoculture

```{r echo=TRUE, warning=FALSE, fig.cap="**Graph. 8. Tradeoff Rubber-Archidendron j.-Parkia s. over Rubber Monoculture**",}
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result2, 
                                    vars = "trade_off_FL",
                                    method = 'boxplot',
                                    colors = "aquamarine3",
...
```{r echo=TRUE, warning=FALSE, fig.cap="**Graph. 9. Tradeoff Rubber-Archidendron j-Sandoricum k. over Rubber Monoculture**",}
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result3, 
                                    vars = "trade_off_FL",
                                    method = 'boxplot',
                                    colors = "plum3",
...
VIP1 <-plot_pls(pls_result, input_table = mod, threshold = 1)
VIP1
```
...
VIP2 <-plot_pls(pls_result, input_table = mod, threshold = 1)
VIP2
```
...
VIP3 <-plot_pls(pls_result, input_table = mod, threshold = 1)
VIP3
```
...
```{r}
# preparation EVPI
mcSimulation_table <- data.frame(mcSimulation_result1$x, mcSimulation_result1$y[1:4])

# calculation EVPI
evpi <- multi_EVPI(mc = mcSimulation_table, first_out_var = "trade_off_FL")

# plot EVPI
plot_evpi(evpi, decision_vars = "trade_off_FL")
```

```{r}
# preparation EVPI
mcSimulation_table2 <- data.frame(mcSimulation_result2$x, mcSimulation_result2$y[1:4])

# calculation EVPI
evpi2 <- multi_EVPI(mc = mcSimulation_table2, first_out_var = "trade_off_FL")

# plot EVPI
plot_evpi(evpi2, decision_vars = "trade_off_FL")
```
...
# calculation EVPI
evpi3 <- multi_EVPI(mc = mcSimulation_table3, first_out_var = "trade_off_FL")

# plot EVPI
plot_evpi(evpi3, decision_vars = "trade_off_FL")
```
