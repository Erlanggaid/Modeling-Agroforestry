...
first_decision_function <- function(x, varnames) {
  ...
  # Benefit variables Coconut-areca nut
  ...
  ### Calculate the system cost of rice
  ...
  # Fertilizer cost areca nut (pinang)
  ...
  # Fertilizing cost areca nut (pinang)
  ...
  # Harvesting cost Areca nut (pinang)
  ...
  # Calculate the NPV of rice agroforestry (coconut-areca nut (pinang))
  
  # Total Benefit coconut-areca nut (pinang)-maize (Family Labour and Non Family Labour are included using shadow price)
  total_rice_ccpn_benefit <- ((total_rice_benefit + tot_cc_benefit  + tot_pn_benefit + AF_maize_benefit) - 
    ...
    # Total Benefit Rice-areca nut-maize (Family Labour and Non Family Labour are included using shadow price)
  ...
  # NPV Rice-coconut-areca nut (pinang)-maize (Family Labour and Non Family Labour are included using shadow price)
  NPV_rice_ccpn <- discount(total_rice_ccpn_benefit, discount_rate = discount_rate, calculate_NPV = TRUE)
  
  
  # NPV Rice-areca nut-maize (Family Labour and Non Family Labour are included using shadow price)
  NPV_rice_pn <- discount(total_rice_pn_benefit, 
                          discount_rate = discount_rate, calculate_NPV = TRUE)
  
  # NPV Rice monoculture (Family Labour and Non Family Labour are included using shadow price)
  NPV_rice <- discount(total_rice_benefit, discount_rate = discount_rate, calculate_NPV = TRUE)
  
  # Benefit of choosing rice-coconut-areca nut (pinang)-maize with dike over Rice Monoculture without dike
  
  tradeoff_benefit1 <- NPV_rice_ccpn - NPV_rice
  
    # Benefit of choosing rice-areca nut-maize with dike over Rice Monoculture without dike
  tradeoff_benefit2 <- NPV_rice_pn - NPV_rice
  
  # Final NPV of the decision to choose Rice-coconut-areca nut (pinang)-maize with dike and field protection over Rice Monoculture
  
  NPV_tradeoff1 <- discount(tradeoff_benefit1, discount_rate = discount_rate, calculate_NPV = TRUE)
  
  # Final NPV of the decision to choose Rice-areca nut-maize with dike and field protection over Rice Monoculture
  NPV_tradeoff2 <- discount(tradeoff_benefit2, 
                           discount_rate = discount_rate, calculate_NPV = TRUE)
...
```{r echo=TRUE, fig.cap="**Graph. 1 Cash flow Rice-maize-Coconut-Areca palm**", warning=FALSE}
...
```{r echo=TRUE, fig.cap="**Graph. 2 Cash flow Rice-maize-areca palm **", warning=FALSE}
plot_cashflow(mcSimulation_object = mcSimulation_result1,
...
```{r echo=TRUE, fig.cap= "**Graph 3. Cash flow Rice Monoculture**", warning=FALSE}
plot_cashflow(mcSimulation_object = mcSimulation_result1, 
              color_25_75 = "grey60",
              color_5_95 = "grey80",
              color_median = "blue",
...
```{r echo=TRUE, warning=FALSE, fig.cap="**Graph. 4 NPV comparison Rice-based agroforestry over Rice Monoculture**", }
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result1, 
                                    vars = c("rice_ccpn_NPV", "rice_NPV", "rice_pn_NPV"),
                                    method = 'smooth_simple_overlay', 
                                    bins = 150,
                                    old_names = NULL,
                                    new_names = NULL,
                                    colors = c("greenyellow", "darkgreen", "darkgoldenrod3"),
                                    outlier_shape = ".",
...
```{r echo=TRUE, warning=FALSE, fig.cap="**Graph. 7 Tradeoff Rice-based agroforestry over Rice Monoculture**", warning=FALSE}
decisionSupport::plot_distributions(mcSimulation_object = mcSimulation_result1, 
                                    vars = c("trade_off1","trade_off2"),
                                    method = 'boxplot',
...
write.csv(mcSimulation_result1, "simulation_incl.csv")
```
